...
for a in aa:                          # per a in tutte le posizioni della lista
	...
	final_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "Program", "Stazioni_Meteo_Trentino", a, "2018"))  # aggiungere 'csv' o 'xls' in base a quale cartella si vuole creare
	if os.path.exists(final_dir):  # se la cartella 'csv' o 'xls' non esiste già
		shutil.rmtree(final_dir)        # crea la cartella con la path di final_dir
